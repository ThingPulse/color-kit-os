    ...
    if ( uri_load_dsc ) {
        ...
        if( doc.containsKey("cod") ) {
            if ( doc["cod"].as<uint32_t>() == 200 ) {
                ...
                weather_today->wind_speed = doc["wind"]["speed"].as<int>();
...
int weather_fetch_forecast( weather_config_t *weather_config, weather_forecast_t * weather_forecast, weather_forecast_t * hourly_forecast ) {
    char url[512]="";
    int httpcode = -1;
    const char* weather_units_char = weather_config->imperial ? "imperial" : "metric";
    const char* weather_lang = weather_config->lang;
